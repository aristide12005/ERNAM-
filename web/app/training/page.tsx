        ...
        >
            ...
            <div className="mb-6 flex justify-between items-start">
                <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-widest border ${isEnrolled
                    ...
                    {isEnrolled ? "MISSION ACTIVE" : "OPEN FOR DEPLOYMENT"}
                </span>
            </div>
            ...
            <p className="text-gray-500 text-sm mb-6 line-clamp-3 font-medium">
                {course.description_fr || "No objective metadata provided."}
            </p>

            <div className="space-y-3 pt-4 border-t border-white/5 mt-auto">
                <div className="flex items-center gap-2 text-xs font-bold text-gray-500 uppercase tracking-wide">
                    <Calendar className="h-3 w-3 text-blue-500" />
                    <span>Activation: {new Date(course.start_date || new Date()).toLocaleDateString()}</span>
                </div>
            </div>
            ...
            >
                {isEnrolled ? "RESUME DEPLOYMENT" : "ANALYZE OBJECTIVE"}
            ...
            >
                <div>
                    ...
                    <p className="text-gray-500 mt-2 text-lg font-medium">
                        Access your active flight simulations and curriculum modules.
                    </p>
                </div>
            ...
            <section>
                ...
                {availableCourses.length === 0 && enrolledCourses.length === 0 && !loading && (
                    <div className="text-center py-20 border border-dashed border-white/10 rounded-2xl">
                        <p className="text-gray-500 font-medium">No active simulations detected in the local sector.</p>
                    </div>
                )}
            </section>
