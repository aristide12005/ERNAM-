...
import Students from '@/components/dashboard/instructor/Students';
...
import ApplicantReview from '@/components/dashboard/admin/ApplicantReview';
import AcademicManagement from '@/components/dashboard/admin/AcademicManagement';
import ExamManagement from '@/components/dashboard/admin/ExamManagement';
...
import StudentResults from '@/components/dashboard/trainee/StudentResults';
...
function StatusBadge({ status }: { status: string }) {
    ...
    // Use translation if available, else fallback to status string capitalized
                ...
                <AnimatePresence>
                    {menuOpen && (
                        <motion.div
                            ...
                            exit={{ opacity: 0, y: 10 }}
                            className="absolute right-0 top-full mt-2 w-64 bg-popover border border-border rounded-lg shadow-2xl overflow-hidden z-50"
                        >
...
export default function DashboardPage() {
    ...
    if (loading) return <div className="h-screen w-full flex items-center justify-center text-muted-foreground animate-pulse">{t('loading')}</div>;
    ...
    return <div className="p-8 text-white">{t('access_denied')}</div>;
}
...
function TraineeDashboard() {
    ...
    useEffect(() => {
        const fetchSession = async () => {
            // Find a session starting in the last 2 hours or next 15 mins (Simulating "Live")
            const now = new Date();
            ...
            const { data } = await supabase
                ...
                .order('start_time', { ascending: true }) // Get the earliest one that is still "relevant"
                .limit(1)
                .maybeSingle();
        ...
        <div className="flex h-screen bg-[#F8FAFC] overflow-hidden">
                    ...
                    <AnimatePresence mode="wait">
                        {activeView === 'dashboard' && (
                            <motion.div
                                ...
                                className="space-y-8"
                            >
                                <h1 className="text-2xl font-semibold text-gray-900">{t('Common.welcome')}, {profile?.full_name?.split(' ')[0] || 'Trainee'}</h1>
                                ...
                                {activeSession && activeSession.title && (
                                    <motion.div
                                        ...
                                        className="bg-gradient-to-r from-blue-600 to-indigo-700 rounded-2xl p-6 text-white shadow-xl shadow-blue-900/10 flex flex-col md:flex-row items-center justify-between gap-6"
                                    >
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <div className="flex items-center gap-2 mb-1">
                                                    ...
                                                    <span className="text-blue-100 text-xs font-medium">{t('Trainee.started_recently')}</span>
                                                </div>
                                                <h3 className="text-xl font-bold">{activeSession.title}</h3>
                                                <p className="text-blue-100 text-sm flex items-center gap-2 mt-1">
                                                    <Users className="h-4 w-4" /> {t('Trainee.class_in_session')} • {activeSession.location || 'Virtual Room'}
                                                </p>
                                            </div>
                                        </div>

                                        <button
                                            onClick={() => setActiveView('messages')} // For now, direct to messages/messaging where calling lives
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                    <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition-shadow flex items-center gap-4">
                                        <div className="relative h-20 w-20">
                                            {stats && !isNaN(stats.overallProgress) ? (
                                        ...
                                        <div>
                                            <p className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">{t('Trainee.overall_progress')}</p>
                                    ...
                                    <div className="bg-gray-900 p-6 rounded-lg text-white flex items-center gap-4 border border-gray-800 shadow-sm">
                                        <div className="h-14 w-14 bg-white/10 rounded-2xl flex items-center justify-center border border-white/5">
                                            <div className="text-center">
                                                ...
                                                <div className="text-[8px] font-bold uppercase text-blue-300">{t('Trainee.next_exam_days')}</div>
                                            </div>
                                        </div>
                                        <div>
                                            <p className="text-xs font-medium text-blue-400 uppercase tracking-wide mb-1">{t('Trainee.next_exam')}</p>
                                    ...
                                    <div className="bg-white p-6 rounded-lg border border-gray-200 shadow-sm hover:shadow-md transition-shadow flex items-center gap-4">
                                        ...
                                        <div>
                                            <p className="text-xs font-medium text-gray-500 uppercase tracking-wide mb-1">{t('Trainee.attendance_rate')}</p>
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                    <div className="lg:col-span-2 space-y-6">
                                        <div className="flex justify-between items-center px-4">
                                            <h3 className="text-sm font-semibold text-gray-700 uppercase tracking-wide">{t('Trainee.current_courses')}</h3>
                                            <button onClick={() => setActiveView('learning')} className="text-xs font-bold text-blue-600 hover:underline">{t('Trainee.view_catalog')}</button>
                                        </div>
                                            ...
                                            {myCourses.map((course) => (
                                                <div key={course.id} className="bg-white border border-gray-200 p-6 rounded-lg hover:shadow-md hover:border-gray-300 transition-all group">
                                                    <div className="flex justify-between items-start mb-4">
                                                        ...
                                                        <span className="text-xs font-medium text-gray-500 uppercase tracking-wide">{course.progress}% {t('Trainee.completed')}</span>
                                                    </div>
                                    ...
                                    <div className="space-y-6">
                                        <div className="px-4">
                                            <h3 className="text-sm font-semibold text-gray-700 uppercase tracking-wide">{t('Trainee.upcoming_schedule')}</h3>
                                        </div>
                                        <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm space-y-4">
                                            ...
                                            <button onClick={() => setActiveView('schedule')} className="w-full py-3 bg-gray-50 text-gray-700 rounded-md text-sm font-medium hover:bg-gray-900 hover:text-white transition-colors">{t('Trainee.view_calendar')}</button>
                                        </div>
                                    </div>
                                </div>
                            </motion.div>
            ...
            <NotificationsDialog
                ...
                onNavigate={(view, context) => {
                    if (view === 'manage-class' && context?.courseId) {
                        // Trainees likely don't manage classes, but safe to include or ignore
                    }
                    // If we add 'course-view' for trainees later, we can handle it here
                    if (view === 'my-learning') setActiveView('learning');
                }}
            />
        </div>
    );
                        ...
                        {activeView === 'dashboard' && (
                            ...
                            >
                                <motion.div
                                    ...
                                    className="mb-8"
                                >
                                    <h1 className="text-2xl font-semibold text-foreground">{t('Common.welcome')}, {profile?.full_name?.split(' ')[0] || 'Teacher'}</h1>
                                </motion.div>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                                    ...
                                    >
                                        ...
                                        <div>
                                            <p className="text-sm text-muted-foreground font-medium">{t('Trainer.active_students')}:</p>
                                    ...
                                    >
                                        ...
                                        <div>
                                            <p className="text-sm text-muted-foreground font-medium">{t('Trainer.pending_grades')}:</p>
                                    ...
                                    >
                                        ...
                                        <div>
                                            <p className="text-sm text-muted-foreground font-medium">{t('Trainer.next_session')}:</p>
                                            {stats.nextSession ? (
                                ...
                                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                                    ...
                                    <div className="lg:col-span-2">
                                        <div className="flex justify-between items-center mb-6">
                                            <h2 className="text-xl font-semibold text-foreground">{t('Trainer.recent_classes')}</h2>
                                            <button onClick={() => setActiveView('my-classes')} className="text-sm text-primary font-bold hover:underline">{t('Common.view_all')}</button>
                                        </div>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                            ...
                                            <div className="bg-card rounded-xl shadow-sm border border-border p-8 text-center opacity-60">
                                                ...
                                                <p className="text-sm">{t('Trainer.quick_access')}</p>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="lg:col-span-1">
                                        <div className="bg-card rounded-xl shadow-sm border border-border p-6 h-full min-h-[500px]">
                                            <h2 className="text-xl font-semibold text-foreground mb-6">{t('Trainer.needs_attention')}</h2>
                                            <div className="space-y-6">
                                                {notifications.length === 0 ? (
                                                    <div className="text-center py-10 opacity-50">
                                                        <CheckCircle2 className="h-8 w-8 mx-auto mb-2 text-emerald-500" />
                                                        <p className="text-sm">{t('Trainer.all_caught_up')}</p>
                                                    </div>
                                                ) : (
                                                    notifications.map((item) => (
                                                        <div key={item.id} className="pb-6 border-b border-border last:border-0 last:pb-0">
                                                            ...
                                                            <div className="flex gap-2 mb-3">
                                                                ...
                                                                >
                                                                    {item.type === 'priority' ? t('Trainer.grade_now') : t('Trainer.view_detail')}
                                                                </button>
                                                            </div>
                                                        </div>
                                                    ))
                                                ...
                                                <div className="pt-4 text-center">
                                                    ...
                                                    >
                                                        {t('Trainer.view_all_notifications')}
                                                    </button>
                                                </div>
                                            </div>
                                        </div>
            ...
            <div className="flex-1 overflow-y-auto relative">
                ...
                <div className="sticky top-0 z-10 bg-[#050505]/80 backdrop-blur-md px-8 py-4 flex justify-between items-center border-b border-white/5">
                    <div className="flex items-center gap-2 text-sm">
                        <span className="text-gray-500">{t('Common.home')}</span>
                        <span className="text-gray-500">/</span>
                        <span className="text-gray-500">{t('Common.governance')}</span>
                        <span className="text-gray-500">/</span>
                        <span className="text-white">{t('Common.dashboard')}</span>
                    </div>
                    <div className="flex items-center gap-4">
                ...
                <div className="p-8 max-w-7xl mx-auto space-y-8 pb-20">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                                    ...
                                    <div className="flex justify-between items-start">
                                        <div className="space-y-1">
                                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest flex items-center gap-2">
                                                {t('Admin.total_revenue')}: <TrendingUp className="h-3 w-3 text-emerald-500" />
                                            </p>
                                ...
                                <div className="bg-[#1A1A1A] border border-white/5 rounded-xl p-5 space-y-4">
                                    <div className="flex justify-between items-start">
                                        <div className="space-y-1">
                                            <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest flex items-center gap-2">
                                                {t('Admin.system_health')}: <CheckCircle className="h-3 w-3 text-emerald-500" />
                                            </p>
                                    ...
                                    <div className="flex justify-between items-start">
                                        <div className="space-y-1">
                                            <p className="text-[10px] font-bold text-red-300 uppercase tracking-widest flex items-center gap-2">
                                                {t('Admin.audit_alerts')}: <AlertCircle className="h-3 w-3 text-red-500" />
                                            </p>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
                                ...
                                <div className="lg:col-span-3 space-y-8">
                                    <div className="bg-[#141414] border border-white/5 rounded-2xl p-6">
                                        <h3 className="text-sm font-bold text-white mb-6">{t('Admin.enrollment_revenue_trends')}</h3>
                                    ...
                                    <div className="bg-[#141414] border border-white/5 rounded-2xl p-6 text-white">
                                        <div className="flex justify-between items-center mb-6">
                                            <h3 className="text-sm font-bold">{t('Admin.trainee_origin')}</h3>
                                            <button className="text-[10px] font-bold text-gray-500 hover:text-white border border-white/10 px-3 py-1.5 rounded-lg">{t('Common.generate_report')}</button>
                                        </div>
                                ...
                                <div className="lg:col-span-2 space-y-8">
                                    <div className="bg-[#141414] border border-white/5 rounded-2xl overflow-hidden flex flex-col h-full">
                                        <div className="p-6 border-b border-white/5 flex justify-between items-center">
                                            <h3 className="text-sm font-bold text-white uppercase tracking-widest">{t('Admin.recent_activity')}</h3>
                                            <button className="text-[10px] font-bold text-gray-500 hover:text-white border border-white/10 px-3 py-1.5 rounded-lg">{t('Common.generate_report')}</button>
                                        </div>
                                            ...
                                            <table className="w-full text-left text-[11px]">
                                                <thead>
                                                    <tr className="border-b border-white/5 text-gray-500">
                                                        <th className="px-6 py-3 font-bold uppercase">{t('Admin.user')}</th>
                                                        <th className="px-6 py-3 font-bold uppercase">{t('Admin.action')}</th>
                                                        <th className="px-6 py-3 font-bold uppercase">{t('Admin.module')}</th>
                                                        <th className="px-6 py-3 font-bold uppercase">{t('Admin.timestamp')}</th>
                                                        <th className="px-6 py-3 font-bold uppercase">{t('Admin.status')}</th>
                                                    </tr>
                                                </thead>
                    ...
                    {activeView === 'applicants' && <ApplicantReview />}
                    {activeView === 'academics' && <AcademicManagement />}
