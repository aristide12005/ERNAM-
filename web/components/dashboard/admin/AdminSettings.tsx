...
export default function AdminSettings() {
    ...
    const fetchSettings = async () => {
        ...
        if (!error && data) {
            ...
            data.forEach((row: any) => {
                ...
                if (row.key === 'support_email') newSettings.support_email = row.value.value;
    ...
    const handleSave = async () => {
        ...
        const updates = [
            ...
            { key: 'support_email', value: { value: settings.support_email } },
                ...
                <div className="flex-1 bg-[#141414] border border-white/5 rounded-2xl p-8 space-y-8">
                    {activeTab === 'general' && (
                        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                            <h3 className="text-lg font-bold text-white border-b border-white/5 pb-4">General Configuration</h3>
                            ...
                            <div className="space-y-4 pt-4">
                                <div className="flex justify-between items-center p-4 bg-white/5 rounded-xl border border-white/5">
                                    <div>
                                        ...
                                        <div className="text-[10px] text-gray-500">When enabled, only administrators can access the platform.</div>
                                    </div>
                                ...
                                <div className="flex justify-between items-center p-4 bg-white/5 rounded-xl border border-white/5">
                                    <div>
                                        ...
                                        <div className="text-[10px] text-gray-500">Allow new users to create accounts independently.</div>
                                    </div>
                                    <div className="relative inline-flex items-center cursor-pointer">
                                        <input
                                            type="checkbox"
                    ...
                    {activeTab === 'security' && (
                        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
                            <h3 className="text-lg font-bold text-white border-b border-white/5 pb-4">Security & Privacy</h3>

                            <div className="space-y-4">
                                <div className="p-4 bg-red-500/5 border border-red-500/10 rounded-xl">
                                    <h4 className="flex items-center gap-2 text-red-400 font-bold text-xs uppercase mb-2">
                                        <Shield className="h-3 w-3" /> Critical Security Actions
                                    </h4>
                                    <p className="text-[10px] text-gray-500 leading-relaxed mb-4">
                                        These actions affect system-wide access and should be used with caution.
                                    </p>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="p-4 bg-[#0A0A0A] rounded-xl border border-white/5 space-y-3">
                                        <div className="flex items-center gap-2 text-white text-xs font-black uppercase">
                                            <Smartphone className="h-3 w-3 text-blue-500" /> Two-Factor Authentication
                                        </div>
                                        <button className="w-full py-2 bg-blue-600/10 text-blue-500 text-[10px] font-bold rounded-lg border border-blue-600/20">Configure Global OTP</button>
                                    </div>
                                    <div className="p-4 bg-[#0A0A0A] rounded-xl border border-white/5 space-y-3">
                                        <div className="flex items-center gap-2 text-white text-xs font-black uppercase">
                                            <Mail className="h-3 w-3 text-emerald-500" /> Domain Whitelisting
                                        </div>
                                        <button className="w-full py-2 bg-emerald-600/10 text-emerald-500 text-[10px] font-bold rounded-lg border border-emerald-600/20">Manage Allowed Domains</button>
                                    </div>
                                </div>
                            </div>
                        </motion.div>
                    ...
                    <div className="flex justify-end gap-3 pt-8 border-t border-white/5">
                        <button
                            ...
                            disabled={loading}
                        >
                            <RotateCcw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} /> Reset Changes
                        </button>
                        <button
                            onClick={handleSave}
