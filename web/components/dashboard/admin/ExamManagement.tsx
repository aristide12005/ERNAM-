        ...
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                <h2 className="text-2xl font-bold text-white flex items-center gap-2">
                    <Calendar className="h-6 w-6 text-blue-500" /> Exam Management
                </h2>
                {profile?.role === 'admin' && (
                    <button
            ...
            <div className="bg-[#141414] border border-white/5 rounded-2xl overflow-hidden min-h-[400px]">
                {loading ? (
                    <div className="flex flex-col items-center justify-center p-12">
                        ...
                        <p className="text-gray-500 italic">Loading exams...</p>
                    </div>
                ) : exams.length === 0 ? (
                    <div className="flex flex-col items-center justify-center p-12 text-center">
                        <Calendar className="h-12 w-12 text-gray-600 mb-4" />
                        <h3 className="text-lg font-bold text-gray-400">No exams scheduled</h3>
                        <p className="text-sm text-gray-500 mt-1">You haven't scheduled any exams for this session yet.</p>
                    </div>
                ) : (
                                    ...
                                    >
                                        ...
                                        <td className="px-6 py-4 text-gray-400 text-xs">
                                            <div className="flex flex-col gap-1">
                                                <span className="flex items-center gap-1"><Clock className="h-3 w-3" /> Start: {new Date(exam.start_date).toLocaleDateString()}</span>
                                                <span className="flex items-center gap-1"><Clock className="h-3 w-3" /> End: {new Date(exam.end_date).toLocaleDateString()}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-4">
                                            <button
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md">
                        ...
                        >
                            ...
                            <form onSubmit={handleCreate} className="space-y-4">
                                <div>
                                    <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Exam Name</label>
                                    <input required value={newItem.name} onChange={e => setNewItem({ ...newItem, name: e.target.value })} className="w-full bg-black/40 border border-white/10 rounded-lg p-3 text-white focus:border-blue-500 outline-none" placeholder="e.g. Final Semester Exam" />
                                </div>

                                <div>
                                    <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Academic Session</label>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">End Date</label>
                                ...
                                <div className="flex items-center gap-3 p-3 bg-white/5 rounded-lg border border-white/5">
                                    ...
                                    <label htmlFor="publish" className="text-sm font-bold text-white cursor-pointer select-none">Publish Immediately</label>
                                </div>
